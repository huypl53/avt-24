...
logger = get_main_logger(__name__, log_file="./logs/cli_lsk_refactored.log", level=logging.INFO)
...
class LSKProcessor:
    ...
    async def _process_task(self, task: TaskMd, task_type: DetectionTaskType):
        ...
        try:
            ...
            is_valid, error_msg, input_param_dict = await self.task_manager.validate_task_params(task)
            if not is_valid:
            ...
            if not pre_param_conf:
                await self.task_manager.mark_task_failed(task, "Failed to load task configuration")
                return
            
            # Update model parameters
            input_params = self.image_processor.update_model_params(input_param_dict, pre_param_conf)
